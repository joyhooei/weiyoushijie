...
module.exports.afterSave = function(request, response) {
    ...
    //登录200钻。每天领取一次
    Gift.create(customer, 1, 200, 0, 0);

    //在线奖励，每天200钻。
    Gift.create(customer, 2, 200, 0, 0);
    
    //首冲 1500钻+1勋章+1M 金币。 只能领取一次，不再刷新。灰色时点击跳转首冲页面。
    Gift.create(customer, 3, 1500, 1, 1000000);

    //拍卖100钻。每天领取一次。灰色点击直接跳去拍卖页面。
    Gift.create(customer, 4, 100, 0, 0);

    //永久会员/月票 300钻。每天领取一次。灰色是点击跳入会员购买页面（参照道具弹出窗口） 月票默认30天，会显示剩余月票天数，如果是永久则显示永久
    Gift.create(customer, 5, 300, 0, 0);

    //分享100钻。每天任意在微博，微信等地方分享一次就可以领取。灰色时点击跳入分享页面。
    Gift.create(customer, 6, 100, 0, 0);
    
    //秒产每增加一个数量级，就得100个钻石
    Gift.create(customer, 7, 100, 0, 0);
    
    //关注
    Gift.create(customer, 8, 200, 0, 0);
};
