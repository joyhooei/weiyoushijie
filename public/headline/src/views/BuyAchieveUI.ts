class BuyAchieveUI extends eui.Component{
    ...
    private _buy(gold:number, diamond:number): void {
        let newOutput = application.vip.getOutput(this._project.output(this._myProject.level,this._achieve,this._myProject.tool_ratio));
        let oldOutput = application.vip.getOutput(this._project.output(this._myProject.level,this._myProject.achieve,this._myProject.tool_ratio));							
